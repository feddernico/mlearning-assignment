...
Let's try a first model with a random forest using all the predictors

```{r}
library(dplyr)
train_corr <- cor(subset(training.new, select=-classe))
train_corr <- as.data.frame(as.table(train_corr)) %>% arrange(Freq)
train_corr
```
...
Once modeled, let's predict new values from the testing data frame and calculate the confusion matrix on the testing data frame
